...
- **cooldown**: 公開から日が浅い依存バージョンと、同一versionのartifact/source identity差し替えを防ぐ（bot 経由・手動 bump を問わず）
...
| `cooldown-check.yaml` | PR で追加/変更された依存に公開 N 日未満の版、またはnpm artifact / Go `replace` のidentity変更があれば fail。違反内容は **sticky な PR コメント**（bot コメント1つを更新）でも通知され、解消すると ✅ に変わる | `pull_request` |
...
| action | 内容 |
|---|---|
| `actions/cooldown-check` | cooldown ゲート本体（Node 製・依存なし）。対象: npm 系 lockfile（package-lockでは `resolved` / `integrity` も監視）・package.json 厳密指定・go.mod / go.sum（`replace` も監視）・workflows の `uses:` |
